      ...
      vim.keymap.set('n', keymaps.preview_image_popup, function()
        ...
        if curr_entry and curr_entry.fs_type == 'file' then
          ...
          if vim.tbl_contains(supported_image_exts, ext) then
            ...
            local dimensions = vim.fn.systemlist(string.format("identify -format '%%w %%h' %s", vim.fn.shellescape(curr_entry.path)))
            ...
            if vim.fn.has 'mac' == 1 or vim.fn.has 'unix' == 1 then
              ...
              local handle =
                io.popen('stat -f%z ' .. vim.fn.shellescape(curr_entry.path) .. ' || ls -l ' .. vim.fn.shellescape(curr_entry.path) .. " | awk '{print $5}'")
              if handle then
